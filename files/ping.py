...
def save_output( msg, postfix=None, ending="json", tstamp=time.time(), outdir="/monroe/results/"):
    f = NamedTemporaryFile(mode='w+', delete=False, dir=outdir)
    f.write(msg)
    f.close()
    outfile = os.path.join(outdir, "raw_socket_ping.log")
...
def verbose_ping(dest_addr, timeout=2, count=4):
    ...
    for i in range(count):
        str='ping {}...'.format(dest_addr)
        print('ping {}...'.format(dest_addr))
        delay = do_one(dest_addr, timeout)
        if delay == None:
            print('failed. (Timeout within {} seconds.)'.format(timeout))
            str+='failed. (Timeout within {} seconds.)'.format(timeout)
        else:
